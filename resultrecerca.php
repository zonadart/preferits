...
if (isset($_GET['recerca'])) {

  $colname_rcdRecerca = (get_magic_quotes_gpc()) ? $_GET['recerca'] : addslashes($_GET['recerca']);

}

mysql_select_db($database_conn_preferits, $conn_preferits);

$query_rcdRecerca = sprintf("SELECT dat.iddat, dat.adreca, tip.tipus, dat.observacions, dat.login, dat.password FROM dat JOIN tip WHERE dat.adreca LIKE CONCAT('%%', %s, '%%') AND dat.tipus = tip.idtip ORDER BY dat.adreca ASC", GetSQLValueString($colname_rcdRecerca, "text"));
...
<title>Resultat de recerca</title>
